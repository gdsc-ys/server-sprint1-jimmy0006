...
router.post('/',(req,res)=>{
    connection.query(`INSERT INTO normal3.comment (userId, blogId, comment) VALUES (${req.body.userid},${req.body.blogid},'${req.body.comment}');`,(err,rows,fields)=>{
        if(err) throw err;
        console.log('success!');
        res.send('success!');
    });
...
//         });
//         qs=qs.slice(0,-5);
//         connection.query(`SELECT * FROM normal3.comment WHERE ${qs}`,(error,rows,fields)=>{
...
router.delete('/',(req,res)=>{
    ...
    if(Object.keys(req.query).length>0){
        qs=qs.slice(0,-5);
        connection.query(`DELETE FROM normal3.comment WHERE ${qs}`,(error,rows,fields)=>{
            if(error) throw error;
