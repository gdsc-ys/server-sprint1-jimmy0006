...
router.get('/',(req,res)=>{
    ...
    if(req.query.city){
        qs+=`u.city = "${req.query.city}" AND `
    }
    if(req.query.address){
        qs+=`u.address = "${req.query.address}" AND `
    }
    if(Object.keys(req.query).length>0){
        qs=qs.slice(0,-5);
        connection.query(`SELECT c.userId, c.blogId, c.comment, u.name , u.zip,u.city,u.address FROM normal2.comment AS c JOIN normal2.user AS u ON c.userId=u.userId WHERE ${qs}`,(error,rows,fields)=>{
