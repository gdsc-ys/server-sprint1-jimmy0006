...
router.post('/',(req,res)=>{
    connection.query(`INSERT INTO normal2.user (userId, name, zip, city, address) VALUES (${req.body.userid},'${req.body.name}',${req.body.zip},'${req.body.city}',${req.body.address});`,(err,rows,fields)=>{
        if(err) throw err;
        console.log('success!');
        res.send('success!');
    });
...
//         });
//         qs=qs.slice(0,-5);
//         connection.query(`SELECT * FROM normal2.user WHERE ${qs}`,(error,rows,fields)=>{
...
router.delete('/',(req,res)=>{
    ...
    if(Object.keys(req.query).length>0){
        qs=qs.slice(0,-5);
        connection.query(`DELETE FROM normal2.user WHERE ${qs}`,(error,rows,fields)=>{
            if(error) throw error;
